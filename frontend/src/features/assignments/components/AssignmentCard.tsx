...
interface AssignmentCardProps {
    ...
    calculateProgress?: (
        signOn: string,
        signOff: string,
    ) => { elapsedDays: number; totalDays: number; percentage: number };
}
...
export function AssignmentCard({ item, variant, calculateProgress }: AssignmentCardProps) {
    ...
    const progress =
        variant === 'active' && calculateProgress
            ? calculateProgress(item.signOnDate, item.signOffDate)
            : null;

    return (
        <div
            className={`bg-white border-l-4 ${currentStyle.border} border-y border-r border-slate-200 rounded-lg p-4 sm:p-5 flex flex-col gap-4 shadow-sm mb-4 transition-all`}
        >
            <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 pb-2 border-b border-dashed border-slate-100 lg:border-none lg:pb-0">
                <div className="flex items-center gap-3 sm:gap-4 flex-1 min-w-0 w-full">
                    ...
                    <div className="flex-1 min-w-0">
                        <p
                            className="text-[11px] sm:text-xs text-slate-500 font-medium truncate mb-0.5 sm:mb-1"
                            title={`${item.vessel.name} • IMO ${item.vessel.imoNumber} • ${item.vessel.type}`}
                        >
                            {item.vessel.name} <span className="text-slate-300 mx-1">•</span> IMO{' '}
                            {item.vessel.imoNumber}{' '}
                            <span className="text-slate-300 mx-1 hidden sm:inline">•</span>{' '}
                            <span className="block sm:inline sm:text-slate-500 text-[10px] sm:text-xs text-slate-400">
                                {item.vessel.type}
                            </span>
                        </p>

                        <h3
                            className="text-sm sm:text-base font-bold text-slate-900 truncate mb-1.5"
                    ...
                    <div className="flex items-center gap-4 lg:gap-6 shrink-0 lg:ml-4 w-full lg:w-auto">
                        ...
                        <div className="w-full lg:w-[264px] flex items-center justify-between bg-slate-50 lg:bg-transparent p-3 lg:p-0 rounded-lg border border-slate-100 lg:border-none">
                            <div className="flex flex-col items-center flex-1 lg:flex-none lg:w-24 min-w-0">
                                <span className="text-[9px] sm:text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-0.5">
                                    Sign-On
                                </span>
                                <span
                                    className="text-xs sm:text-sm font-bold text-slate-900 truncate w-full text-center"
                                    title={item.signOnPort}
                                >
                                    {item.signOnPort}
                                </span>
            ...
            <div className="flex justify-between items-center mt-1">
                <div
                    className={`inline-flex items-center gap-1.5 px-2.5 py-1 border text-xs font-bold rounded-md shadow-sm ${currentStyle.badgeBg} ${currentStyle.badgeText}`}
                >
